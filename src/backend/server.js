...
const SESSION_COOKIE = "bidifi_session";
const WORKSPACE_COOKIE = "bidifi_workspace";
const SESSION_TTL_MS = Math.max(60 * 60 * 1000, Number(process.env.BIDIFI_SESSION_TTL_MS || 7 * 24 * 60 * 60 * 1000));
const SECURITY_SECRET = String(process.env.BIDIFI_SESSION_SECRET || "").trim() || crypto.randomBytes(48).toString("hex");
const ALLOWED_ORIGINS = new Set(
  String(process.env.BIDIFI_ALLOWED_ORIGINS || "")
    .split(",")
    .map((x) => x.trim())
    .filter(Boolean)
);
...
if (IS_PRODUCTION && !process.env.BIDIFI_SESSION_SECRET) {
  console.warn("[BIDIFI SECURITY] BIDIFI_SESSION_SECRET is not set. Sessions will reset after process restart. Set a strong secret in production.");
}
...
app.use(
  cors({
    ...
    allowedHeaders: ["Origin", "X-Requested-With", "Content-Type", "Accept", "Authorization", "X-BIDIFI-Request-ID"],
  })
);
...
// Production upload/workspace responses must never be served from an old cache.
// This is important on Vercel/CDN deployments where the upload request and the
// following workspace request can otherwise observe different response state.
...
function setCookie(res, name, value, options = {}) {
  const parts = [`${name}=${encodeURIComponent(value)}`, `Path=${options.path || "/"}`, `Max-Age=${Math.floor((options.maxAge ?? SESSION_TTL_MS) / 1000)}`, "HttpOnly", "SameSite=Lax"];
  if (IS_PRODUCTION || options.secure) parts.push("Secure");
  if (options.sameSite === "None") {
    parts[parts.length - 1] = "SameSite=None";
    if (!parts.includes("Secure")) parts.push("Secure");
  }
  if (options.clear) {
    const filtered = parts.filter((x) => !x.startsWith("Max-Age="));
    filtered.push("Max-Age=0");
    res.append("Set-Cookie", filtered.join("; "));
    return;
  }
...
function clearCookie(res, name) {
  res.append("Set-Cookie", `${name}=; Path=/; Max-Age=0; HttpOnly; SameSite=Lax${IS_PRODUCTION ? "; Secure" : ""}`);
}
...
const AI_CONCURRENCY = Math.max(1, Number(process.env.BIDIFI_AI_CONCURRENCY || 3));
let activeAiJobs = 0;
const aiWaiters = [];
async function acquireAiSlot() {
  if (activeAiJobs < AI_CONCURRENCY) { activeAiJobs += 1; return; }
...
async function withAiSlot(task) {
  await acquireAiSlot();
  try { return await task(); } finally { releaseAiSlot(); }
}

const workspaceALS = new AsyncLocalStorage();

function currentWorkspaceId() {
  return workspaceALS.getStore()?.workspaceId || (IS_PRODUCTION ? "system-denied" : "dev-local");
}

function currentAuthUserId() {
  return workspaceALS.getStore()?.userId || null;
}

function isSafeApiPath(pathname) {
  return pathname === "/api/status" || pathname === "/api/health" || pathname.startsWith("/api/auth/");
}

function securityWorkspaceMiddleware(req, res, next) {
  const origin = String(req.headers.origin || "");
  if (origin && !ALLOWED_ORIGINS.has(origin)) {
    return res.status(403).json({ success: false, message: "Origin is not allowed." });
  }

  const requestId = String(req.headers["x-bidifi-request-id"] || `req_${Date.now()}_${crypto.randomBytes(6).toString("hex")}`);
  req.bidifiRequestId = requestId;
  res.setHeader("X-BIDIFI-Request-ID", requestId);
  ...
  let user = null;
  try { user = typeof getAuthenticatedUser === "function" ? getAuthenticatedUser(req) : null; } catch (_) {}

  let workspaceId = user ? `user:${user.id}` : "";
  let userId = user?.id || null;

  if (!workspaceId) {
    const cookieWorkspace = verifySignedCookieValue(cookies[WORKSPACE_COOKIE]);
    if (cookieWorkspace && /^anon_[a-f0-9]{64}$/.test(cookieWorkspace)) workspaceId = cookieWorkspace;
  }

  if (!workspaceId) {
    if (IS_PRODUCTION && !isSafeApiPath(req.path)) {
      return res.status(401).json({ success: false, message: "Authentication required." });
    }
    workspaceId = IS_PRODUCTION ? "system-denied" : "dev-local";
    if (!IS_PRODUCTION) {
      const anon = `anon_${crypto.randomBytes(32).toString("hex")}`;
      workspaceId = anon;
      setCookie(res, WORKSPACE_COOKIE, encodeSignedCookieValue(workspaceId), { maxAge: 30 * 24 * 60 * 60 * 1000 });
    }
  }

  if (["POST", "PUT", "PATCH", "DELETE"].includes(req.method) && req.path.startsWith("/api/") && !isSafeApiPath(req.path) && origin && !ALLOWED_ORIGINS.has(origin)) {
    return res.status(403).json({ success: false, message: "Cross-site request blocked." });
  }

  const authKey = userId ? `user:${userId}` : `${clientIp(req)}:${req.path}`;
  const limit = req.path.startsWith("/api/auth/") ? 12 : req.path.includes("upload") ? 30 : 180;
  const windowMs = req.path.startsWith("/api/auth/") ? 15 * 60 * 1000 : 60 * 1000;
  if (!rateLimit(authKey, limit, windowMs)) {
    res.setHeader("Retry-After", String(Math.ceil(windowMs / 1000)));
    return res.status(429).json({ success: false, message: "Too many requests. Please try again shortly." });
  }

  return workspaceALS.run({ workspaceId, userId }, () => next());
}

app.use(securityWorkspaceMiddleware);
...
function getFeedback() {
  ...
  try {
    ...
    return Array.isArray(parsed)
      ? parsed.filter((item) => !item?.ownerWorkspaceId || item.ownerWorkspaceId === currentWorkspaceId())
      : [];
  } catch (error) {
    console.error(
...
/* ============================================================
   PER-WORKSPACE ISOLATED MEMORY STORES
   ============================================================ */
...
class ScopedStore {
  ...
  key(id) {
    return `${currentWorkspaceId()}::${String(id)}`;
  }
  set(id, value) {
    const ownerWorkspaceId = currentWorkspaceId();
    if (value && typeof value === "object" && !value.ownerWorkspaceId) value.ownerWorkspaceId = ownerWorkspaceId;
    this.map.set(this.key(id), value);
    return this;
  }
  get(id) {
    return this.map.get(this.key(id));
  }
  has(id) {
    return this.map.has(this.key(id));
  }
  delete(id) {
    return this.map.delete(this.key(id));
  }
  values() {
    const prefix = `${currentWorkspaceId()}::`;
    const values = [];
    for (const [key, value] of this.map.entries()) if (key.startsWith(prefix)) values.push(value);
    return values.values();
  }
  entries() {
    const prefix = `${currentWorkspaceId()}::`;
    const entries = [];
    for (const [key, value] of this.map.entries()) if (key.startsWith(prefix)) entries.push([key.slice(prefix.length), value]);
    return entries.values();
  }
  keys() {
    const prefix = `${currentWorkspaceId()}::`;
    const keys = [];
    for (const key of this.map.keys()) if (key.startsWith(prefix)) keys.push(key.slice(prefix.length));
    return keys.values();
  }
  get size() {
    const prefix = `${currentWorkspaceId()}::`;
    let count = 0;
    for (const key of this.map.keys()) if (key.startsWith(prefix)) count += 1;
    return count;
  }
}
...
const tenderStore =
  new ScopedStore("tenders");

const bidderStore =
  new ScopedStore("bidders");

const analysisStore =
  new ScopedStore("analyses");

const reportStore =
  new ScopedStore("reports");
...
function getAuthenticatedUser(req) {
  ...
  const cookieToken = verifySignedCookieValue(cookies[SESSION_COOKIE]);
...
async function validateUploadedFileSignature(file) {
  ...
  if (ext === ".docx") return buffer.subarray(0, 2).toString("ascii") === "PK";
  return false;
}
...
async function extractFileContent(
  ...
  if (!validateUploadedFileSignature(file)) {
  ...
  if (
    ...
    const extractedText = safeString(parsed.text).trim();
    if (extractedText.length >= 20) {
      return {
        text: extractedText,
        pages: Number(parsed.numpages) || 1,
    ...
    if (openai) {
      try {
        ...
        const response = await withAiSlot(() => openai.responses.create({
          model: process.env.BIDIFI_OCR_MODEL || OPENAI_MODEL,
          input: [{
            role: "user",
            content: [
              {
                type: "input_text",
                text: "Extract the readable text from this procurement document. Preserve numbers, dates, certifications, clauses, thresholds, table values and section boundaries. Return only the extracted document text; do not summarize or invent missing content.",
              },
              {
                type: "input_file",
                filename: cleanFilename(file.originalname) || "document.pdf",
                file_data: dataUrl,
              },
            ],
          }],
        }), 90000, "OCR");
        const ocrText = safeString(response?.output_text).trim();
        ...
        console.warn("[BIDIFI OCR] OCR fallback failed:", ocrError?.message || "unknown error");
      }
    }
...
`;
  ...
  try {
    const response = await withTimeout(
      withAiSlot(() => openai.responses.create({
        model: OPENAI_MODEL,
        input: prompt
      })),
...
function validateAIItem(
   ...
   * ------------------------------------
   * AI cannot change a deterministic requirement decision here.
   * Evidence/status stay grounded in bidder-document evidence and the
   * requirement-specific deterministic evaluator.
   */
  return localItem;
}
/* ============================================================
...
app.post("/api/auth/register", (req, res) => {
  try {
    ...
    setCookie(res, SESSION_COOKIE, encodeSignedCookieValue(token), { maxAge: SESSION_TTL_MS });
...
app.post("/api/auth/logout", (req, res) => {
  ...
  const bearer = header.startsWith("Bearer ") ? header.slice(7).trim() : "";
  const cookieToken = verifySignedCookieValue(cookies[SESSION_COOKIE]);
  if (bearer) SESSION_STORE.delete(bearer);
  if (cookieToken) SESSION_STORE.delete(cookieToken);
  clearCookie(res, SESSION_COOKIE);
  clearCookie(res, WORKSPACE_COOKIE);
...
async function handleTenderUpload(files, res, uploadedBy = {}) {
  ...
  return res.json({
    ...
    uploaded: returnedTenders,
    records: returnedTenders,
    ...
    recordId: firstTender?.id || null,
    id: firstTender?.id || null,
...
app.post(
  ...
  async (
    ...
    try {
      ...
      if (analysis?.id) {
        const ownedAnalysis = analysisStore.get(analysis.id);
        if (!ownedAnalysis) {
          return res.status(403).json({ success: false, message: "You do not have access to this analysis." });
        }
        const ownedTender = tenderStore.get(ownedAnalysis.tenderId);
        if (!ownedTender) {
          return res.status(403).json({ success: false, message: "You do not have access to this tender." });
        }
      }
  ...
  (
    ...
    res
      ...
      .json({
        ...
        message:
          IS_PRODUCTION
            ? "Internal server error. Use the request ID when contacting support."
...
module.exports = app;
setInterval(() => { const cutoff = Date.now() - 20 * 60 * 1000; for (const [key, value] of rateBuckets) if (value.startedAt < cutoff) rateBuckets.delete(key); }, 10 * 60 * 1000).unref?.();
