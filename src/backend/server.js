    ...
    /* ============================================================
       ...
       If pdf-parse returns little/no text (typical scanned/image PDFs),
       send the original PDF directly to the vision-capable flagship model.
       This preserves tables and page structure better than guessing from an
       empty text layer. It is a fallback, so normal text PDFs remain fast.
       ============================================================ */
    async function extractPdfTextWithVisionFallback(file, buffer, pageCount) {
      if (!openai || !buffer || !Buffer.isBuffer(buffer)) return "";

      const maxPages = Math.max(1, Number(process.env.BIDIFI_OCR_MAX_PAGES || pageCount || 1));
      const maxBytes = Math.max(1, Number(process.env.BIDIFI_OCR_MAX_BYTES || 25 * 1024 * 1024));
      if (buffer.length > maxBytes) return "";

      try {
        const dataUrl = `data:application/pdf;base64,${buffer.toString("base64")}`;
        const prompt = `
You are the document extraction/OCR layer for BIDIFI procurement verification.
Extract ONLY text and table content that is visibly present in this PDF.
Do not summarize, interpret, infer, classify, or invent anything.
Preserve numbers, units, dates, percentages, certificate IDs, clause wording,
negative statements, and table row relationships exactly as far as possible.
Keep page/section order. If a table is present, use clear row/column text.
Return plain text only. This text will later be matched against tender
requirements, so factual fidelity is more important than readability.
PDF page count reported by parser: ${maxPages}.
`;
          ...
          openai.responses.create({
            ...
            input: [{
              ...
              content: [
                ...
                { type: "input_file", filename: cleanFilename(file.originalname || "document.pdf"), file_data: dataUrl }
              ]
            }]
          }),
          Number(process.env.BIDIFI_OCR_TIMEOUT_MS || 30000),
          "PDF visual OCR"
        );

        return normalizeWhitespace(response?.output_text || "");
      } catch (error) {
        console.warn("[BIDIFI] PDF visual OCR fallback skipped:", error?.message || error);
        return "";
      }
    }
    ...
    async function extractFileContent(
      ...
      const skipVisionOCR = options?.skipVisionOCR === true;
      const ext =
        path
          .extname(
            file.originalname
      ...
      if (
        ...
        const parsedText = safeString(parsed.text);
        const pages = Number(parsed.numpages) || 1;
        const sparseText = parsedText.replace(/\s+/g, " ").trim();
        const needsVisionOCR =
          String(process.env.BIDIFI_PDF_VISION_OCR || "true").toLowerCase() !== "false" &&
          (sparseText.length < Math.max(250, pages * 80) ||
            !/[A-Za-z0-9]{3,}/.test(sparseText));

        let finalText = parsedText;
        let ocrUsed = false;

        // Tender upload must not wait for slow visual OCR. Other document
        // consumers keep the original synchronous OCR behaviour.
        if (needsVisionOCR && !skipVisionOCR) {
          const visionText = await extractPdfTextWithVisionFallback(file, buffer, pages);
          if (visionText && visionText.length > finalText.length) {
        ...
        return {
          ...
          ocrUsed,
          ocrPending: needsVisionOCR && skipVisionOCR,
          sourceBuffer: skipVisionOCR ? buffer : null,
        };
      }
    ...
    function buildDirectEvidenceFallbacks(requirement, bidderDocuments) {
      ...
      for (const document of safeArray(bidderDocuments)) {
        ...
        for (const piece of pieces) {
          ...
          if (!requirementEvidenceContextCompatible(req, piece)) continue;
    ...
    /* ============================================================
       STRICT REQUIREMENT/EVIDENCE CONTEXT COMPATIBILITY
       Prevent unrelated numeric/text evidence from being reused across
       different requirement families (for example EMD evidence for turnover,
       delivery days for bid validity, or EMD evidence for warranty).
       ============================================================ */
    function requirementEvidenceContextCompatible(requirement, evidence) {
      const req = normalizeRequirementObject(requirement);
      const r = normalizeForCompare(reqTextForSnippet(req));
      const e = normalizeForCompare(evidence);
      if (!r || !e) return false;

      const any = (patterns) => patterns.some((p) => p.test(r));
      const has = (patterns) => patterns.some((p) => p.test(e));

      // Financial thresholds must be evidenced with a financial concept.
      if (req.structured?.requiredAmount !== null &&
          any([/turnover/i, /revenue/i, /net worth/i, /annual turnover/i, /financial/i, /sales/i, /gross income/i])) {
        if (!has([/turnover/i, /revenue/i, /net worth/i, /annual turnover/i, /financial statement/i, /sales/i, /gross income/i, /profit/i])) return false;
      }

      // Bid validity is distinct from delivery/project timelines.
      if (any([/bid validity/i, /bid valid/i, /offer validity/i, /offer valid/i, /validity period/i, /valid for \d+ days/i])) {
        if (!has([/bid validity/i, /bid valid/i, /offer validity/i, /offer valid/i, /validity period/i, /valid for \d+ days/i, /valid for \d+ working days/i])) return false;
      }

      // Delivery/supply/implementation timelines must contain delivery context.
      if (req.structured?.requiredDays !== null &&
          any([/delivery/i, /supply/i, /dispatch/i, /installation/i, /implementation/i, /deployment/i, /execution timeline/i, /project timeline/i])) {
        if (!has([/delivery/i, /supply/i, /dispatch/i, /installation/i, /implementation/i, /deployment/i, /execution timeline/i, /project timeline/i])) return false;
      }

      // Warranty/guarantee is its own evidence family.
      if (any([/warranty/i, /guarantee/i, /guarantee period/i])) {
        if (!has([/warranty/i, /guarantee/i, /guarantee period/i])) return false;
      }

      // Penalty / liquidated damages must be explicitly about damages/penalty.
      if (any([/penalty/i, /liquidated damages/i, /damages/i, /ld\b/i])) {
        if (!has([/penalty/i, /liquidated damages/i, /damages/i, /ld\b/i, /fine/i])) return false;
      }

      // Tax-inclusive pricing must mention tax/price/delivery-charge concepts.
      if (any([/prices? include/i, /inclusive of taxes/i, /taxes included/i, /gst included/i, /including taxes/i])) {
        if (!has([/price/i, /tax/i, /gst/i, /inclusive/i, /delivery charges?/i, /freight/i])) return false;
      }

      // Certification requirements must match the NAMED certification, not merely any certificate.
      if (req.structured?.requiredCertifications?.length) {
        if (!has([/certif/i, /certificate/i, /iso\b/i, /soc\s*2/i, /cmmi/i, /pci\s*dss/i, /appraisal/i])) return false;
        const requiredCerts = safeArray(req.structured.requiredCertifications);
        const normalizedEvidence = normalizeForCompare(e);
        const namedMatch = requiredCerts.some((cert) => {
          const c = normalizeForCompare(cert);
          if (!c) return false;
          if (normalizedEvidence.includes(c)) return true;
          if (/iso\s*9001/.test(c) && /iso\s*9001/.test(normalizedEvidence)) return true;
          if (/iso\s*27001/.test(c) && /iso\s*27001/.test(normalizedEvidence)) return true;
          if (/soc\s*2/.test(c) && /soc\s*2/.test(normalizedEvidence)) return true;
          if (/cmmi/.test(c) && /cmmi/.test(normalizedEvidence)) return true;
          return false;
        });
        if (!namedMatch) return false;
      } else if (any([/certification/i, /certificate/i, /iso\b/i, /soc\s*2/i, /cmmi/i, /pci\s*dss/i])) {
        if (!has([/certif/i, /certificate/i, /iso\b/i, /soc\s*2/i, /cmmi/i, /pci\s*dss/i, /appraisal/i])) return false;
      }

      // CMMI evidence must actually mention CMMI/appraisal.
      if (any([/cmmi/i])) {
        if (!has([/cmmi/i, /appraisal/i])) return false;
      }

      // Local support requirements need support/local-office/SLA evidence, not
      // a financial statement that happens to come from the same PDF.
      if (any([/local\s+support/i, /support\s+office/i, /support\s+center/i, /24x7/i, /24\/7/i, /sla/i, /help\s*desk/i])) {
        if (!has([/support/i, /support\s+office/i, /support\s+center/i, /local\s+office/i, /sla/i, /24x7/i, /24\/7/i, /help\s*desk/i, /service\s+desk/i])) return false;
      }

      // 24x7 support must actually mention the support/availability commitment.
      if (req.structured?.requires24x7) {
        if (!has24x7Support(e) || !has([/support/i, /help ?desk/i, /service desk/i, /availability/i, /uptime/i])) return false;
      }

      /*
       * HARD REQUIREMENT-FAMILY ISOLATION
       * ---------------------------------
       * The same bidder PDF can contain turnover, CMMI, warranty, OEM,
       * support, etc. A sentence is usable only when it belongs to the
       * requirement family being checked. This prevents a turnover sentence
       * from becoming Local Service Center evidence and a CMMI sentence from
       * becoming Data Center/Tier evidence merely because they are in the
       * same document.
       */
      const category = normalizeForCompare(firstNonEmpty(
        req.category,
        inferRequirementCategory(req.structured?.attributes || [])
      ));
      const reqFamilyText = normalizeForCompare([
        req.title,
        req.requirement,
        req.description,
        req.requirementText
      ...
      if (
        category === "support" ||
        req.structured?.attributes?.includes("support") ||
        req.structured?.attributes?.includes("availability") ||
        /local service center|support center|support office|technical center|help desk|service desk|24\s*[x\/×]\s*7|sla/i.test(reqFamilyText)
      ) {
        if (!has([
          /support/i,
          /service\s*center/i,
      ...
      if (
        category === "experience" ||
        req.structured?.attributes?.includes("experience") ||
        /past performance|track record|relevant experience|years.*experience|experience.*years|similar projects?/i.test(reqFamilyText)
      ) {
        if (!has([
          /experience/i,
          /experienced/i,
      ...
      if (
        category === "security" ||
        req.structured?.attributes?.includes("security") ||
        /data center|tier[-\s]?\d|security logs|security compliance/i.test(reqFamilyText)
      ) {
        if (
          /data center|tier[-\s]?\d|security logs|hosting/i.test(reqFamilyText) &&
          !has([
            /data\s*center/i,
            /tier\s*[- ]?\d/i,
            /security\s+logs?/i,
      ...
      if (/oem|original equipment manufacturer|gold\s+partner|platinum\s+partner|partner certificate/i.test(reqFamilyText)) {
        if (!has([
          /oem/i,
          /original\s+equipment\s+manufacturer/i,
          /partner/i,
          /gold\s+partner/i,
          /platinum\s+partner/i,
          /certificate/i
        ])) return false;
      }

      if (/on[- ]site engineers?|dedicated.*engineers?|network engineers?/i.test(reqFamilyText)) {
        if (!has([
          /engineers?/i,
          /on[- ]site/i,
          /dedicated/i,
    ...
    function semanticCandidateAllowed(
      ...
      if (!requirementEvidenceContextCompatible(req, text)) {
        return false;
      }
    ...
    function scoreEvidenceCandidate(
      ...
      if (!requirementEvidenceContextCompatible(req, text)) return 0;
    ...
    function evaluateCMMIRequirement(req, evidence) {
      const text = normalizeWhitespace(reqTextForSnippet(req));
      if (!/\bcmmi\b/i.test(text)) return null;

      const requiredMatch = text.match(/\bcmmi(?:\s*level)?\s*(\d+)\b/i);
      const requiredLevel = requiredMatch ? Number(requiredMatch[1]) : 0;
      const negative = /(?:not\s+(?:applicable|available|held|certified)|pending|no\s+cmmi|without\s+cmmi)/i.test(evidence);
      if (negative) return { name: "CMMI", status: "FAIL", reason: "The bidder evidence states that CMMI certification/appraisal is unavailable or pending." };

      const levels = [...String(evidence).matchAll(/\bcmmi(?:\s*level)?\s*(\d+)\b/gi)].map(m => Number(m[1]));
      const highest = levels.length ? Math.max(...levels) : null;
      const hasCertificate = /\b(?:certificate|certification|appraisal|appraisal\s+certificate|appraisal\s+id)\b/i.test(evidence);

      if (highest !== null && highest >= requiredLevel && hasCertificate) {
        return { name: "CMMI", status: "PASS", reason: `Bidder evidence shows CMMI Level ${highest}, meeting the required Level ${requiredLevel || highest} or higher.` };
      }
      if (highest !== null && requiredLevel && highest < requiredLevel) {
        return { name: "CMMI", status: "FAIL", reason: `Bidder evidence shows CMMI Level ${highest}, below the required Level ${requiredLevel}.` };
      }
      if (highest !== null || /\bcmmi\b/i.test(evidence)) {
        return { name: "CMMI", status: "UNKNOWN", reason: "CMMI evidence was found, but the required level/certificate could not be fully verified." };
      }
      return null;
    }
    ...
    function authoritativeBidderProof(
      ...
      for (const d of docs) {
        ...
        const relevantNegative = sentences.find((s) => {
          ...
          if (!requirementEvidenceContextCompatible(req, s)) return false;
    ...
    /* ============================================================
       REQUIREMENT MATCH-PERCENT ENGINE
       ============================================================
       Compliance is evidence coverage, not AI confidence.

       Rules:
       - Exact/full evidence = 100%.
       - Numeric threshold evidence is proportional to the actual value.
       - Multi-part requirements are scored by covered required components.
       - No evidence = 0%.
       - Explicit negative evidence = 0%, unless a measurable partial value
         exists (for example CMMI 2 when CMMI 3 is required).
       - Confidence is kept separate from compliance percentage.
       ============================================================ */

    function extractEvidenceText(item) {
      return unique([
        ...safeArray(item?.evidenceCandidates).map((x) => x?.text),
        item?.evidence,
        item?.reason
      ].map(cleanEvidenceText).filter(Boolean));
    }

    function extractCMMILevel(text) {
      const matches = [...normalizeForCompare(text).matchAll(
        /\\bcmmi(?:\\s*level)?\\s*(\\d+)\\b/gi
      )].map((m) => Number(m[1]));
      return matches.length ? Math.max(...matches) : null;
    }

    function ratioToPercent(actual, required, higherIsBetter = true) {
      const a = Number(actual);
      const r = Number(required);
      if (!Number.isFinite(a) || !Number.isFinite(r) || r <= 0) return null;
      const ratio = higherIsBetter ? (a / r) : (r / a);
      if (!Number.isFinite(ratio)) return null;
      return clamp(ratio * 100, 0, 100);
    }

    function calculateRequirementMatchPercent(item) {
      const req = normalizeRequirementObject(item);
      const evidence = extractEvidenceText(item);
      const status = safeString(item?.status || item?.matchStatus).toUpperCase().replace(/[\\s-]+/g, "_");
      ...
      // Explicit negative bidder evidence is never a positive match. This is
      // requirement-scoped so a "NOT AVAILABLE" statement for CMMI/security
      // cannot accidentally become evidence for another requirement.
      const negativeEvidence = /\b(?:not\s+available|not\s+provided|not\s+applicable|pending(?:\s+certification)?|no\s+(?:cmmi|certificate|certification|support|office|evidence)|unsupported|does\s+not\s+meet|does\s+not\s+have|cannot\s+provide|unable\s+to\s+provide|expired|revoked|invalid|non[-\s]?compliant)\b/i.test(combined);
      if (negativeEvidence && requirementEvidenceContextCompatible(req, combined)) return 0;

      // Project-count requirements are evaluated from qualifying project facts,
      // not from the "within N years" window appearing in the tender text.
      // This prevents the required 5-year window from being mistaken for the
      // bidder's actual experience duration.
      const reqText = [req.title, req.requirement, req.description, req.requirementText]
        .filter(Boolean).join(" ");
      const projectRule = reqText.match(/\b(?:at\s+least|minimum(?:\s+of)?|minimum)\s*(\d+)\s+(?:similar\s+|relevant\s+|major\s+|enterprise\s+|commercial\s+|government\s+|public\s+sector\s+|psu\s+)*(?:projects?|contracts?|assignments?|engagements?|implementations?)\b/i);
      if (projectRule) {
        const requiredProjects = Number(projectRule[1]);
        const projectMatches = [
          ...combined.matchAll(/\b(\d+)\s+(?:major\s+|similar\s+|relevant\s+|enterprise\s+|commercial\s+|government\s+|public\s+sector\s+|psu\s+)*(?:projects?|contracts?|assignments?|engagements?|implementations?)\b/gi),
          ...combined.matchAll(/\b(?:completed|executed|delivered|implemented|successfully\s+completed|successfully\s+delivered)\s+(\d+)\s+(?:major\s+|similar\s+|relevant\s+|enterprise\s+|commercial\s+|government\s+|public\s+sector\s+|psu\s+)*(?:projects?|contracts?|assignments?|engagements?|implementations?)\b/gi)
        ];
        const certificateProjectMatch = combined.match(/(?:completion\s+certificates?|certificates?|proof|records?|references?|reference\s+projects?)\s+(?:for|of|covering)\s+(\d+)\s+(?:major\s+|similar\s+|relevant\s+|enterprise\s+|commercial\s+|government\s+|public\s+sector\s+|psu\s+)*(?:projects?|contracts?|assignments?|engagements?|implementations?)\b/i);
        const actualProjects = Math.max(0, ...projectMatches.map((m) => Number(m[1])), certificateProjectMatch ? Number(certificateProjectMatch[1]) : 0);
        if (actualProjects > 0) return Number((Math.min(actualProjects / requiredProjects, 1) * 100).toFixed(2));
      }

      // Financial / turnover / revenue / net-worth threshold.
      if (structured.requiredAmount !== null) {
        ...
        if (amounts.length) {
          const actual = Math.max(...amounts);
          return Number((ratioToPercent(actual, structured.requiredAmount, true) ?? 0).toFixed(2));
        }
      }

      // Experience / company-age / other year threshold.
      if (structured.requiredYears !== null) {
        const years = evidence
          .map(extractYears)
          .filter((x) => Number.isFinite(x));

        if (years.length) {
          const actual = Math.max(...years);
          return Number((ratioToPercent(actual, structured.requiredYears, true) ?? 0).toFixed(2));
        }
      }

      // CMMI level is ordinal: Level 2 against Level 3 is 66.67%, while
      // Level 3/4/5 against Level 3 is capped at 100%.
      const requiredCmmi = safeArray(structured.requiredCertifications)
        .map((x) => normalizeForCompare(x).match(/cmmi(?:\\s*level)?\\s*(\\d+)/i))
        .filter(Boolean)
        .map((m) => Number(m[1]));

      if (requiredCmmi.length) {
        const actual = extractCMMILevel(combined);
        if (actual !== null) {
          return Number((ratioToPercent(actual, Math.max(...requiredCmmi), true) ?? 0).toFixed(2));
        }
      }

      // Response/SLA limits: a slower but measurable response is a partial
      // evidence match, not an automatic failure. Example: required 2 hours,
      // bidder 4 hours => 50% requirement match. This is generic for any SLA.
      const requirementHoursMatch = reqText.match(/\b(?:within|under|max(?:imum)?|response(?:\s+time)?(?:\s+of)?|sla(?:\s+response)?(?:\s+of)?)\s*(?:a|an)?\s*(\d+(?:\.\d+)?)\s*(minutes?|mins?|hours?|hrs?)\b/i);
      const evidenceHoursMatches = [...combined.matchAll(/\b(?:within|under|response(?:\s+time)?(?:\s+of)?|sla(?:\s+response)?(?:\s+of)?|response)\s*(?:a|an)?\s*(\d+(?:\.\d+)?)\s*(minutes?|mins?|hours?|hrs?)\b/gi)];
      if (requirementHoursMatch && evidenceHoursMatches.length) {
        const reqValue = Number(requirementHoursMatch[1]) * (/minutes?|mins?/i.test(requirementHoursMatch[2]) ? 1 : 60);
        const actualValues = evidenceHoursMatches.map((m) => Number(m[1]) * (/minutes?|mins?/i.test(m[2]) ? 1 : 60)).filter(Number.isFinite);
        if (actualValues.length && reqValue > 0) {
          const actual = Math.min(...actualValues);
          return Number((Math.min(reqValue / actual, 1) * 100).toFixed(2));
        }
      }

      // Response/delivery/warranty duration limits.
      if (structured.requiredDays !== null) {
        const days = evidence.map(extractDays).filter((x) => Number.isFinite(x));
        if (days.length) {
          const actual = Math.min(...days);
          return Number((ratioToPercent(actual, structured.requiredDays, false) ?? 0).toFixed(2));
        }
      }

      // Percentage thresholds.
      if (structured.requiredPercentage !== null) {
        const values = evidence.map(extractPercentage).filter((x) => Number.isFinite(x));
        if (values.length) {
          const actual = Math.max(...values);
          return Number((ratioToPercent(actual, structured.requiredPercentage, true) ?? 0).toFixed(2));
        }
      }

      // Multi-part controls/certifications: score the exact required components
      // that are actually present in bidder evidence.
      const requiredComponents = [
        ...safeArray(structured.attributes).map((x) => `attribute:${x}`),
        ...safeArray(structured.requiredCloudPlatforms).map((x) => `cloud:${x}`),
        ...safeArray(structured.requiredSecurityControls).map((x) => `security:${x}`),
        ...safeArray(structured.requiredCertifications).map((x) => `certification:${x}`)
      ];
      ...
      if (requiredComponents.length) {
        const covered = safeArray(item?.coveredComponents);
        const coveredCount = requiredComponents.filter((x) => covered.includes(x)).length;
        if (coveredCount || safeArray(item?.missingComponents).length) {
          return Number(((coveredCount / requiredComponents.length) * 100).toFixed(2));
        }

        // Certification fallback when the adjudicator did not expose component
        // IDs but the evidence contains named certificates.
        if (structured.requiredCertifications.length) {
          const matched = structured.requiredCertifications.filter((cert) =>
            extractCertification(combined).some((found) =>
              normalizeForCompare(found) === normalizeForCompare(cert)
            )
          ).length;
          return Number(((matched / structured.requiredCertifications.length) * 100).toFixed(2));
        }
      }

      // 24/7 support + response SLA can be measured by component coverage.
      if (structured.requires24x7) {
        const has247 = has24x7Support(combined);
        const hasSupport = /\\b(?:support|help\\s*desk|service\\s*desk|technical)\\b/i.test(combined);
        const checks = [has247, hasSupport];
        return Number(((checks.filter(Boolean).length / checks.length) * 100).toFixed(2));
      }

      // If the deterministic adjudicator already established a full pass,
      // preserve 100%. For a review/partial result, use evidence similarity as
      // a conservative fallback instead of inventing full compliance.
      ...
      if (overlap > 0) {
        return Number((clamp(overlap * 100, 0, 89)).toFixed(2));
      }

      return 0;
    }

    function confidenceFromMatchPercent(matchPercent, item) {
      const p = clamp(Number(matchPercent), 0, 100);
      const evidenceCount = safeArray(item?.evidenceCandidates).length;
      if (p === 0 && evidenceCount === 0) return 92;
      if (p >= 90) return 96;
      if (p >= 85) return 91;
      if (p >= 70) return 88;
      if (p >= 50) return 84;
      if (p >= 30) return 80;
      return evidenceCount ? 76 : 92;
    }

    function statusFromMatchPercent(matchPercent, currentStatus, item) {
      const p = clamp(Number(matchPercent), 0, 100);
      const evidence = extractEvidenceText(item);

      // Keep an explicit hard failure only when the evidence itself proves a
      // failure and there is no measurable partial value to represent.
      const hardNegative =
        currentStatus === "NON_COMPLIANT" &&
        /\\b(?:not\\s+available|not\\s+provided|unsupported|does\\s+not|cannot|unable|expired|revoked|invalid|non[-\\s]?compliant|short\\s+by|below\\s+the|required.*not)\\b/i.test(
          evidence.join(" ")
        );

      // 90%+ is a deterministic COMPLIANT decision. Anything below 90%
      // remains REVIEW when evidence exists; match percentage and confidence
      // tell the user how much of the requirement was actually supported.
      // This deliberately avoids turning every partial match below 85% into a
      // false NON_COMPLIANT result.
      if (p >= 90) return "COMPLIANT";
      if (evidence.length) return "REVIEW";
      if (hardNegative) return "REVIEW";
      return "MISSING";
    }
    ...
    function extractRequirementWeight(item) {
      const direct = [
        item?.weight,
        item?.weightage,
        item?.matchWeight,
      ...
      // Prefer explicit evaluation/weight language so a technical threshold
      // such as "5%" is not mistaken for the requirement's score weight.
      const explicit = text.match(/(?:match\s*weight|weight(?:age)?|expected\s*score|score)\s*[:=]?\s*(\d+(?:\.\d+)?)\s*%/i);
      if (explicit) return Number(explicit[1]);

      // Some extracted tender tables place the weight as the final percentage
      // token in the requirement row. Only use it when it is the final token.
      const tail = text.match(/(?:^|\s)(\d+(?:\.\d+)?)\s*%\s*$/);
      if (tail) return Number(tail[1]);

      return null;
    }

    function requirementEvidenceScore(item) {
      const matchPercent = Number.isFinite(Number(item?.matchPercent))
        ? Number(item.matchPercent)
        : calculateRequirementMatchPercent(item);
      return clamp(matchPercent, 0, 100) / 100;
    }
    ...
    function calculateResultSummary(
      ...
      const extractedWeights = requirements.map(extractRequirementWeight);
      const usableWeights = extractedWeights.filter((value) => Number.isFinite(value) && value >= 0);
      const hasTenderWeights = usableWeights.length === total && usableWeights.some((value) => value > 0);
      const totalWeight = hasTenderWeights
        ? usableWeights.reduce((sum, value) => sum + value, 0)
        : total;

      const weightedCompliance = requirements.reduce((sum, item, index) => {
        const weight = hasTenderWeights ? extractedWeights[index] : 1;
        return sum + weight * requirementEvidenceScore(item);
      }, 0);

      const compliancePercentage = totalWeight > 0
        ? Number(((weightedCompliance / totalWeight) * 100).toFixed(2))
        : 0;

      const averageMatchPercent = total
        ? Number((
            requirements.reduce((sum, item) => {
              const p = Number.isFinite(Number(item?.matchPercent))
                ? Number(item.matchPercent)
                : calculateRequirementMatchPercent(item);
              return sum + clamp(p, 0, 100);
            }, 0) / total
          ).toFixed(2))
        : 0;
      ...
      return {
        ...
        complianceWeightTotal: Number(totalWeight.toFixed(2)),

        weightedCompliance: Number(weightedCompliance.toFixed(2)),
    ...
    */
    ...
    /* ============================================================
       EVIDENCE AGGREGATION / SECOND-CHANCE PASS
       ============================================================
       Build the final evidence set from BOTH the strict evaluator and a
       deterministic document scan. This is intentionally generic: it never
       names a particular bidder/PDF. It prevents a valid sentence from being
       lost simply because the first semantic pass ranked another sentence
       higher. Each candidate is still subjected to requirement-family
       isolation before it can contribute to the score.
       ============================================================ */
    function aggregateRequirementEvidence(requirement, bidderDocuments, proofCandidates = []) {
      const req = normalizeRequirementObject(requirement);
      const all = [];
      const seen = new Set();
      const add = (candidate) => {
        const cleaned = cleanBidderEvidenceCandidate(candidate);
        if (!cleaned) return;
        if (!requirementEvidenceContextCompatible(req, cleaned.text)) return;
        try {
          if (!bidderEvidenceCandidateAllowed(req, cleaned)) return;
        } catch (_) { return; }
        const key = normalizeForCompare(cleaned.text);
        if (!key || seen.has(key)) return;
        seen.add(key);
        all.push(cleaned);
      };

      safeArray(proofCandidates).forEach(add);
      try {
        buildDirectEvidenceFallbacks(req, bidderDocuments).forEach(add);
      } catch (_) {}
      try {
        findEvidenceCandidates(req, bidderDocuments).forEach(add);
      } catch (_) {}

      // Keep the strongest evidence first.
      return all
        .map((x) => ({ ...x, score: Number(x.score || 0) }))
    ...
    function finalAuthoritativeReAdjudication(result, bidderDocuments) {
      ...
      const items = originalItems.map((item, index) => {
        ...
        // Evidence is ALWAYS rebuilt from deterministic bidder-only proof plus
        // a second deterministic document scan. AI evidence is never trusted
        // as final evidence.
        const evidenceCandidates = aggregateRequirementEvidence(
          requirement,
          docs,
          proof?.evidenceCandidates
        );

        // If a strict evaluator says MISSING but the second scan found valid
        // bidder evidence, do NOT preserve the stale MISSING decision.
        const finalEvidence = evidenceCandidates;
        const finalStatus = finalEvidence.length ? status : "MISSING";
        ...
        const matchPercent = calculateRequirementMatchPercent(provisionalItem);
        const adjudicatedStatus = statusFromMatchPercent(
          matchPercent,
          finalStatus,
          provisionalItem
        );

        const confidence = confidenceFromMatchPercent(
          matchPercent,
          provisionalItem
        );
        ...
        const reason =
          matchPercent >= 90
            ? "The bidder evidence sufficiently matches the tender requirement."
            : matchPercent > 0
              ? `The bidder evidence partially matches this requirement (${matchPercent.toFixed(2)}%).`
              : "No trustworthy bidder-document evidence was found for this requirement.";
        ...
        return {
          ...
          result: adjudicatedStatus,
          decision: adjudicatedStatus,
          complianceStatus: adjudicatedStatus,
          verificationStatus: adjudicatedStatus,
          matchStatus: adjudicatedStatus,
          compliant: adjudicatedStatus === "COMPLIANT",
          matchPercent: Number(matchPercent.toFixed(2)),
          evidenceMatchPercent: Number(matchPercent.toFixed(2)),
          confidence,
          ...
          evidenceCandidates: finalEvidence,
          evidence: finalEvidence.slice(0, 5).map((e) => e?.text).filter(Boolean).join(" | "),
          aiVerified: false,
          verificationAuthority: "BIDIFI deterministic requirement-specific adjudicator + evidence match percentage"
        };
      });
    ...
    async function analyzeCompliance(
      ...
      combined.requirementsAnalysis =
        safeArray(reAdjudicated.requirementsAnalysis).map((item) => {
          ...
          const matchPercent = calculateRequirementMatchPercent({
            ...item,
            evidenceCandidates: evidence,
            evidence: evidence.map((e) => e?.text).filter(Boolean).join(" ")
          });
          const recalculatedStatus = statusFromMatchPercent(
            matchPercent,
            status,
            { ...item, evidenceCandidates: evidence }
          );
          const confidence = confidenceFromMatchPercent(
            matchPercent,
            { ...item, evidenceCandidates: evidence }
          ...
          return {
            ...
            aiConfidence: confidence,
            evidenceConfidence: confidence,
            compliant: recalculatedStatus === "COMPLIANT",
            status: recalculatedStatus,
            result: recalculatedStatus,
            decision: recalculatedStatus,
            complianceStatus: recalculatedStatus,
            verificationStatus: recalculatedStatus,
            matchStatus: recalculatedStatus
          };
        });
    ...
    app.get(
      ...
      (
        ...
        res.json({
          ...
          engineVersion:
            "BIDIFI-AUTHORITATIVE-2026-09-20-R17-MATCH-PERCENT-90-COMPLIANT",
    ...
    async function enrichTenderAfterUpload(tender, file, sourceBuffer) {
      if (!tender || !file || !Buffer.isBuffer(sourceBuffer)) return;
      if (!openai || path.extname(file.originalname || "").toLowerCase() !== ".pdf") {
        tender.ocrPending = false;
        tender.extractionStatus = tender.requirements?.length ? "READY" : "TEXT_READY";
        return;
      }

      try {
        console.log(`[BIDIFI] Background OCR started for ${tender.id}.`);
        const visionText = await extractPdfTextWithVisionFallback(
          { ...file, path: file.path },
          sourceBuffer,
          Number(tender.pages) || 1
        );

        if (visionText && visionText.length > safeString(tender.text).length) {
          tender.text = visionText;
          const explicit = dedupeRequirements(extractExplicitTenderRows(visionText));
          const local = dedupeRequirements(extractRequirementsLocally(visionText));
          const rows = explicit.length ? explicit : local;
          tender.requirements = rows.map((item, index) => ({
            ...item,
            id: `REQ-${String(index + 1).padStart(2, "0")}`
          }));
          tender.extractionStatus = tender.requirements.length ? "READY" : "TEXT_READY";
          tender.extractionMethod = explicit.length
            ? "source-first explicit requirement extraction (background OCR)"
            : "deterministic requirement extraction (background OCR)";
          tender.ocrUsed = true;
        }

        tender.ocrPending = false;
        tender.ocrCompletedAt = nowIso();
        console.log(`[BIDIFI] Background OCR completed for ${tender.id}.`);
      } catch (error) {
        tender.ocrPending = false;
        console.warn(`[BIDIFI] Background OCR failed for ${tender.id}:`, error?.message || error);
      }
    }
    ...
    async function handleTenderUpload(files, res, uploadedBy = {}) {
      ...
      const results = await Promise.all(
        safeArray(files).map(async (file) => {
          try {
            // Do not block the upload response on visual OCR. The tender is
            // inserted into the library first; scanned-PDF OCR continues in
            // the background and enriches the same tender object.
            const content = await extractFileContent(file, { skipVisionOCR: true });
            const sourceText = safeString(content.text).trim();
            const hasUsableText = sourceText.length >= 20;

            const explicit = hasUsableText
              ? dedupeRequirements(extractExplicitTenderRows(sourceText))
              : [];

            const local = hasUsableText
              ? dedupeRequirements(extractRequirementsLocally(sourceText))
              : [];
            ...
            const tender = {
              ...
              type: content.type,
              requirements,
              extractionStatus: requirements.length
                ? "READY"
                : (content.ocrPending ? "OCR_PENDING" : "TEXT_READY"),
              extractionMethod: explicit.length
                ? "source-first explicit requirement extraction"
                : (content.ocrPending ? "pending background OCR" : "deterministic requirement extraction"),
              ocrPending: Boolean(content.ocrPending),
              ocrUsed: Boolean(content.ocrUsed),
              tenderSummary: "",
              tenderTitle: "",
              issuingAuthority: "",
              uploadedBy: {
            ...
            // Start slow OCR only after the tender has been saved. This makes
            // the first upload complete once, instead of requiring repeated
            // clicks while OCR is still running.
            if (content.ocrPending && Buffer.isBuffer(content.sourceBuffer)) {
              void enrichTenderAfterUpload(tender, file, content.sourceBuffer);
            }
    ...
    app.post(
      ...
      async (
        ...
        try {
          ...
          /*
           * EXPLICIT BUTTON EXTRACTION — SOURCE-FIRST ONLY
           *
           * The initial tender upload already uses the deterministic, source-first
           * requirement extraction path. Keep the manual "Extract requirements"
           * button on that exact same path so clicking it cannot introduce extra
           * AI-generated requirements, merged rows, or requirements that are not
           * actually present as distinct procurement conditions in the tender.
           *
           * IMPORTANT: Do not call runAIRequirementExtraction() here. The upload
           * flow remains untouched; this change applies only to this POST endpoint.
           */
          const explicitRequirements = dedupeRequirements(
            extractExplicitTenderRows(tenderText)
          );
          const localRequirements = dedupeRequirements(
            extractRequirementsLocally(tenderText)
          );
          const previousRequirements = safeArray(tender?.requirements);

          // Prefer the same source-first result used during upload. If the tender
          // has explicit REQ/CLAUSE rows, those rows are authoritative. Otherwise
          // use the deterministic extractor. Existing requirements are only the
          // final fallback when the current text cannot yield any rows.
    ...
    try {
      ...
      throw error;
          ...
          res.json({
            ...
            engineVersion:
              "BIDIFI-AUTHORITATIVE-2026-09-20-R16-EVIDENCE-MATCH-PERCENT-WEIGHTED",
